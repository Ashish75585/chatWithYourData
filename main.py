...
# Import openai and huggingface as main LLM services
...
def sidebar_and_documentChooser():
    ...
    with st.sidebar:
        st.caption(
            "🚀 A retrieval augmented generation chatbot powered by 🔗 Langchain, Cohere, OpenAI and 🤗"
        )
        st.write("")
...
def langchain_document_loader():
    """
    Crete document loaders for TXT and CSV files.
...
def select_embeddings_model():
    """Select embeddings models: OpenAIEmbeddings or HuggingFaceEmbeddings."""
    embeddings = None
    if st.session_state.LLM_provider == "OpenAI":
        embeddings = OpenAIEmbeddings(api_key=st.session_state.openai_api_key, model='text-embedding-ada-002')
